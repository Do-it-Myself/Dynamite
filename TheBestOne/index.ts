...
class Bot {
    constructor() {
        ...
        this.tertiaryTransitions = new Map();
    ...
    getTertiaryTransition(key: [Round | null, Round | null, Round | null, Move]) {
        return this.tertiaryTransitions.get(JSON.stringify(key)) ?? 0;
    }
    ...
    incrementTertiaryTransition(key: [Round | null, Round | null, Round | null, Move]) {
        this.tertiaryTransitions.set(JSON.stringify(key), this.getTertiaryTransition(key) + 1);
    }
    ...
    setTertiaryTransition(key: [Round | null, Round | null, Round | null, Move], value: number) {
        this.tertiaryTransitions.set(JSON.stringify(key), value);
    }
    ...
    tertiaryTransitions: Map<string, number>;
    ...
    update(game: Game) {
        ...
        const secondaryDecayFactor = 0.95;
        const tertiaryDecayFactor = 0.95;
        ...
        const fourRoundsAgo = game.rounds[game.rounds.length - 4] ?? null
        ...
        this.transitions.forEach((v, k) => {
            this.transitions.set(k, v * secondaryDecayFactor);
        });

        // three prev states
        this.incrementTertiaryTransition([fourRoundsAgo, threeRoundsAgo, twoRoundsAgo, oneRoundAgo.p2]);
        this.transitions.forEach((v, k) => {
            this.transitions.set(k, v * tertiaryDecayFactor);
        });
    ...
    predictOpponentsMove(game: Game): /** relative probability */ number[] {
        ...
        const tertiaryOccurenceFactor = 10;
        ...
        for (const move of possibleMoves) {
            ...
            const tertiaryOccurences = this.getTertiaryTransition([threeRoundsAgo, twoRoundsAgo, oneRoundAgo, move])
            relativeProbabilities.push(occurences + secondaryOccurences * secondaryOccurenceFactor + tertiaryOccurences*tertiaryOccurenceFactor);
        }
    ...
    public clear() {
        ...
        this.tertiaryTransitions.clear();
